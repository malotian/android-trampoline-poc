...
class PathClassifierTest {
    ...
    fun `login path with query params still routes to Auth`() {
        val result = PathClassifier.classify(uri("http://www.staples.com/login?returnUrl=/p/123"))
        assertEquals(RouteBucket.Auth, result)
    }

    // --- Auth callback bucket (custom scheme, NOT http) ---
    ...
    @Test
    fun `http callback path does NOT get treated as AuthCallback`() {
        // Confirm the classifier doesn't accidentally special-case it as Auth or
        // AuthCallback — it should fall through to Unknown.
        val result = PathClassifier.classify(uri("http://www.staples.com/callback?code=abc123"))
    ...
    fun `unsubscribe path routes to BrowserOnly`() {
        val result = PathClassifier.classify(uri("http://www.staples.com/unsubscribe?id=999"))
        assertEquals(RouteBucket.BrowserOnly, result)
    }

    @Test
    fun `legal path routes to BrowserOnly`() {
        val result = PathClassifier.classify(uri("http://www.staples.com/legal/terms-of-use"))
        assertEquals(RouteBucket.BrowserOnly, result)
    }
    ...
    @Test
    fun `unknown path does not match any known bucket (should fall back to browser)`() {
        val result = PathClassifier.classify(uri("http://www.staples.com/mystery-new-path"))
        assertTrue(result is RouteBucket.Unknown)
    }

    @Test
    fun `path with no leading data at all does not crash`() {
        val result = PathClassifier.classify(uri("http://www.staples.com"))
        assertTrue(result is RouteBucket.Unknown)
    }
}
